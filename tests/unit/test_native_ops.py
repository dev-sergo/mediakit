...
async def test_compress_quality_int(sample_jpg: Path, tmp_path: Path) -> None:
    ...
    result = await compress(
        CompressParams(input=sample_jpg, output=out, format=ImageFormat.jpeg, quality=60)
    )
    # low quality should produce smaller file
    result_high = await compress(
        CompressParams(
            input=sample_jpg, output=tmp_path / "high.jpg", format=ImageFormat.jpeg, quality=95
        )
    )
...
async def test_compress_max_width_clamps(sample_jpg: Path, tmp_path: Path) -> None:
    ...
    await compress(
        CompressParams(input=sample_jpg, output=out, format=ImageFormat.webp, max_width=300)
    )
    img = Image.open(out)
...
async def test_resize_fit(sample_jpg: Path, tmp_path: Path) -> None:
    ...
    await resize(
        ResizeParams(input=sample_jpg, output=out, width=400, height=400, mode=ResizeMode.fit)
    )
    img = Image.open(out)
...
async def test_resize_fill_exact(sample_jpg: Path, tmp_path: Path) -> None:
    ...
    await resize(
        ResizeParams(input=sample_jpg, output=out, width=400, height=400, mode=ResizeMode.fill)
    )
    img = Image.open(out)
    assert img.width == 400
    assert img.height == 400
...
async def test_resize_pad_exact(sample_jpg: Path, tmp_path: Path) -> None:
    ...
    await resize(
        ResizeParams(input=sample_jpg, output=out, width=500, height=500, mode=ResizeMode.pad)
    )
    img = Image.open(out)
